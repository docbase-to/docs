...
sequenceDiagram
    ...
    participant DB as Docbase
    participant Q as Message Queue
    participant W as Webhook Handler

    App->>DB: POST /webhooks/subscribe
    DB-->>App: { id, secretKey }

    Note over DB: Event occurs (document.created)

    DB->>Q: Publish WebhookDispatched
    Q->>W: WebhookTriggered message
    W->>App: POST /your-endpoint (signed)
    App-->>W: 200 OK

    Note over W: On failure → retry 1h, 3h, 6h, 12h, 24h
```
