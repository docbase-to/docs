...
sequenceDiagram
    ...
    Note over App,Docbase: Later, an event occurs...

    Docbase->>App: POST /your-endpoint
    Note over Docbase: X-Docbase-Signature: t=<timestamp>,v1=<hmac>

    App->>App: Verify signature (timestamp + HMAC-SHA256)
    App->>App: Check eventId for duplicates
    App-->>Docbase: 200 OK — process async

    Note over App,Docbase: If no 200 response...

    Docbase->>App: Retry 1 — after 1h
    Docbase->>App: Retry 2 — after 3h
    Docbase->>App: Retry 3 — after 6h
    Docbase->>App: Retry 4 — after 12h
    Docbase->>App: Retry 5 — after 24h
